...
{
	$data=array("where"=>"comment_id","where_value"=>$_GET['comment_id']);
	$me->delete(COMMENT,$data);
	$count=array(
	...
	$total_coment=$db->select(COMMENT,$count);
	 ...
	 $field=array('deal_id'=>$_GET['deal_id']);
	...
	<tbody>
       <?php
		$data=array(
				 ...
				 $resulet= $db->select(COMMENT,$data);
				 $sl=1;
		...
		<tr>
			...
			echo $getusername[0]['name']?></td>
			<td align="center"><?php echo $v['comments']?></td>
		<td align="center"><div class='timeago' title='<?php  echo $v['commented_at'] ?>'</div></td>
            ...
            <td align="center"> <!--<a data-placement="top" class="btn small bg-blue-alt tooltip-button" href="<?php $_SERVER['PHP_SELF']?>?action=edit&user_id=<?php echo $v['user_id']?>" data-original-title="Edit">
                 ...
                 <a data-placement="top" class="btn small bg-red tooltip-button" href="<?php $_SERVER['PHP_SELF']?>?action=delete&comment_id=<?php echo $v['comment_id']?>&deal_id=<?php echo $v['deal_id']?>" onClick="javascript:var message=confirm('Do you really want to delete');if(message==true){return true;}else {return false;}" data-original-title="Remove">
